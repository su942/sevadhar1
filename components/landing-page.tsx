...
import { Wrench, Users, ShieldCheck, Clock, IndianRupee, Star, ChevronRight, CheckCircle2, MapPin, Quote } from 'lucide-react';
...
const LandingPage: React.FC<LandingPageProps> = ({ onGetStarted }) => {
    ...
    // Select popular services from constants
    const popularServices = SERVICES.slice(0, 6).map(service => ({
        title: service.title,
        image: service.image,
        desc: service.description,
        rating: `${service.rating} (${Math.floor(Math.random() * 10) + 1}k)`,
    ...
    return (
        <div className="min-h-screen bg-white font-sans text-gray-900 overflow-x-hidden flex flex-col">
            ...
            <nav className="flex items-center justify-between px-6 py-4 md:px-12 sticky top-0 bg-white/90 backdrop-blur-md z-50 border-b border-gray-100 transition-all shadow-sm">
                <div className="flex items-center gap-3 cursor-pointer h-12" onClick={handleGetStarted}>
            ...
            <div className="flex-1">
                ...
                <section className="py-24 bg-gray-50 overflow-hidden">
                    <div className="max-w-6xl mx-auto px-6 mb-12 flex justify-between items-end">
                        ...
                        <button onClick={handleGetStarted} className="text-[#FF6B35] font-bold flex items-center gap-1 hover:gap-2 transition-all">
                    ...
                    <div className="flex gap-6 overflow-x-auto pb-8 px-6 no-scrollbar snap-x max-w-6xl mx-auto">
                        {popularServices.map((s, i) => (
                            <div key={i} onClick={handleGetStarted} className="min-w-[280px] bg-white rounded-2xl shadow-sm border border-gray-100 cursor-pointer hover:shadow-xl hover:translate-y-[-4px] transition-all snap-center group overflow-hidden">
                                {/* Image Header */}
                                <div className="h-40 overflow-hidden relative">
                                    <img src={s.image} loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" alt={s.title} />
                ...
                <section className="py-24 px-6 bg-gray-50">
                    <div className="max-w-6xl mx-auto">
                        <div className="text-center mb-16">
                            ...
                            <p className="text-gray-500 mt-2">Real stories from satisfied customers in Khopoli</p>
                        </div>
                        <div className="grid md:grid-cols-3 gap-8">
                            {[
                                { name: 'Priya Sharma', location: 'Shilphata', text: 'Sevadhar made finding a reliable cleaner so easy! The service was prompt and professional.', rating: 5 },
                                { name: 'Rajesh Patel', location: 'Veena Nagar', text: 'Great platform for local services. Booked an electrician in minutes and got the work done perfectly.', rating: 5 },
                                { name: 'Meera Desai', location: 'Khopoli Station', text: 'Love the transparency and direct payment. No more middlemen hassles!', rating: 5 }
                            ].map((testimonial, idx) => (
                                <div key={idx} className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 hover:shadow-lg transition-shadow">
