...
import { Star, Phone, ShieldCheck, Play, Pause, CalendarClock, Quote } from 'lucide-react';
      ...
      {provider.reviews && provider.reviews.length > 0 && (
         <div className="mt-3 pt-3 border-t border-gray-50 flex items-start gap-2">
            <div className="mt-0.5">
               <div className="bg-orange-50 text-[#FF6B35] w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold">
                  <Quote size={10} className="fill-current" /> 
               </div>
            </div>
            <div className="flex-1">
               <p className="text-xs text-gray-600 italic leading-relaxed">"{provider.reviews[0].comment}"</p>
               <p className="text-[10px] text-gray-400 mt-1 font-medium">- {provider.reviews[0].userName}, {provider.reviews[0].date}</p>
            </div>
         </div>
      )}
        ...
        {!showNumber ? (
            <div className="flex flex-col gap-3">
                <button 
                    onClick={handleCall}
                    disabled={adTimer > 0}
                    className="w-full bg-[#FF6B35] text-white py-3 rounded-lg font-semibold hover:bg-orange-600 transition-colors flex items-center justify-center gap-2 disabled:bg-gray-300 text-sm shadow-sm"
                >
                    {adTimer > 0 ? (
                ...
                <button 
                    onClick={onBook}
                    className="w-full bg-white border border-gray-300 text-gray-700 py-3 rounded-lg font-semibold hover:bg-gray-50 transition-colors flex items-center justify-center gap-2 text-sm"
                >
